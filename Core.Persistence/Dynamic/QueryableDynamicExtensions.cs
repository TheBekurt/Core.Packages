...
public static class QueryableDynamicFilterExtensions
{
    ...
    private static IQueryable<T> Filter<T>(IQueryable<T> queryable, Filter filter)
    {
        ...
        var values = filters.Select(f => f.Value).ToArray<object?>();
    ...
    private static IQueryable<T> Sort<T>(IQueryable<T> queryable, IEnumerable<Sort> sort)
    {
        ...
        var ordering = string.Join(separator: ",", values: sorts.Select(s => $"{s.Field} {s.Direction}"));
    ...
    public static string Transform(Filter filter, IList<Filter> filters)
    {
        ...
        {
            return !_logics.Contains(filter.Logic) ? throw new ArgumentException("Invalid Logic") : $"{where} {filter.Logic} ({string.Join(separator: $" {filter.Logic} ", value: filter.Filters.Select(f => Transform(f, filters)).ToArray())})";
        }
